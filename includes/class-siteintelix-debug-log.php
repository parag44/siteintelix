 ...
 *
 * Reads from randomized protected SiteIntelix storage.
 *
 * @package SiteIntelix
 * @since   1.1.0
 */
...
class SITEINTELIX_Debug_Log {
	...
	public static function get_data( $limit = 300 ) {
		...
		$data = array(
			...
			'enabled'  => class_exists( 'SITEINTELIX_WP_Config' ) && SITEINTELIX_WP_Config::is_debug_enabled_in_file(),
			'size'     => 0,
			'entries'  => array(),
			'counts'   => array(),
		);
	 ...
	 */
	public static function get_path_for_mode( $method = 'wp_config' ) {
	 ...
	 *
	 * @param string $method Retained for backward compatibility.
	 * @return string
	 */
	public static function get_mode_label( $method = 'wp_config' ) {
		return __( 'WordPress debug logging to protected SiteIntelix storage', 'siteintelix' );
	}
