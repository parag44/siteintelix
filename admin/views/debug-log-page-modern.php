...
?>
<div class="wrap siteintelix-wrap siteintelix-debug-ui <?php echo $siteintelix_group_similar ? '' : 'is-ungrouped-view'; ?>" id="siteintelix-debug-log-page">
	<header class="siteintelix-header sitx-debug-modern-header">
		<div class="siteintelix-header__content">
			<div class="siteintelix-header__title-group">
				...
				<div class="siteintelix-header__text">
					...
					<p class="siteintelix-header__desc"><?php esc_html_e( 'Review errors and system notices from your environment.', 'siteintelix' ); ?></p>
				</div>
			</div>
			...
			<div class="siteintelix-header__actions">
				...
				<form method="post" action="<?php echo esc_url( $siteintelix_clear_url ); ?>" class="sitx-debug-header-form">
				...
				<?php if ( $siteintelix_log_has_file ) : ?>
					<a class="sitx-btn sitx-btn--primary" href="<?php echo esc_url( $siteintelix_download_url ); ?>" target="_blank" rel="noopener noreferrer">
	...
	<section class="sitx-debug-summary" aria-label="<?php esc_attr_e( 'Debug log summary', 'siteintelix' ); ?>">
		<?php foreach ( $siteintelix_summary_cards as $siteintelix_card ) : ?>
			<article class="sitx-summary-card sitx-summary-card--<?php echo esc_attr( $siteintelix_card['key'] ); ?>">
				...
				<svg class="sitx-summary-card__spark" viewBox="0 0 80 34" aria-hidden="true" focusable="false">
					<polyline points="2,25 14,25 22,14 31,24 43,24 52,6 63,27 78,27" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"></polyline>
				</svg>
			</article>
		<?php endforeach; ?>
	</section>

	<section class="sitx-debug-status-grid sitx-debug-status-grid--single" aria-label="<?php esc_attr_e( 'Debug log status', 'siteintelix' ); ?>">
		<div class="sitx-status-card">
			<span class="sitx-status-card__icon dashicons <?php echo 'wp_config' === $siteintelix_active_method ? 'dashicons-editor-code' : 'dashicons-shield'; ?>" aria-hidden="true"></span>
			<div>
			...
			<a class="sitx-debug-btn sitx-debug-btn--secondary" href="<?php echo esc_url( $siteintelix_settings_url ); ?>"><?php esc_html_e( 'Switch Mode', 'siteintelix' ); ?></a>
		</div>
	</section>

	<section class="sitx-filter-panel" aria-label="<?php esc_attr_e( 'Debug log filters', 'siteintelix' ); ?>">
		<div class="sitx-filter-panel__top">
			<form class="sitx-search-field" method="get" action="<?php echo esc_url( admin_url( 'admin.php' ) ); ?>">
				...
				<input type="search" id="siteintelix-log-search" name="siteintelix_log_search" value="<?php echo esc_attr( $siteintelix_search_query ); ?>" data-siteintelix-global-search placeholder="<?php esc_attr_e( 'Search logs...', 'siteintelix' ); ?>" autocomplete="off">
			...
			<button type="button" class="sitx-debug-btn sitx-debug-btn--secondary" data-sitx-more-filters aria-expanded="false">
				<span class="dashicons dashicons-filter" aria-hidden="true"></span><?php esc_html_e( 'More Filters', 'siteintelix' ); ?>
			</button>
		</div>
		<div class="sitx-filter-panel__bottom" data-sitx-advanced-filters>
			...
			<button type="button" class="sitx-filter-link sitx-filter-link--clear" id="siteintelix-clear-filters">
				<span class="dashicons dashicons-no-alt" aria-hidden="true"></span><?php esc_html_e( 'Clear Filters', 'siteintelix' ); ?>
			</button>
			<button type="button" class="sitx-debug-btn sitx-debug-btn--secondary" id="siteintelix-saved-filters">
				<span class="dashicons dashicons-bookmark" aria-hidden="true"></span><?php esc_html_e( 'Saved Filters', 'siteintelix' ); ?>
			</button>
		</div>
	</section>

	<section class="sitx-log-section" aria-label="<?php esc_attr_e( 'Log entries', 'siteintelix' ); ?>">
				...
				<p>
					...
					if ( ! $siteintelix_log_has_file ) {
						...
						printf(
							...
							esc_html__( 'No log groups matched "%s".', 'siteintelix' ),
							esc_html( $siteintelix_search_query )
						);
					} else {
		...
		<?php else : ?>
			<div class="sitx-log-list" data-sitx-log-list>
				<?php foreach ( $siteintelix_page_groups as $siteintelix_index => $siteintelix_group ) : ?>
					...
					?>
					<article class="sitx-log-card sitx-log-card--<?php echo esc_attr( $siteintelix_level ); ?>"
						data-log-card
						data-group-card
					...
					>
						<div class="sitx-log-card__summary">
							...
							<div class="sitx-log-card__actions">
								<button type="button" class="sitx-debug-btn sitx-debug-btn--secondary" data-sitx-toggle-details aria-expanded="false" aria-controls="<?php echo esc_attr( $siteintelix_group_id ); ?>">
									<span class="dashicons dashicons-visibility" aria-hidden="true"></span><span data-sitx-toggle-label><?php esc_html_e( 'View', 'siteintelix' ); ?></span>
								</button>
								<button type="button" class="sitx-icon-btn" data-sitx-toggle-details aria-label="<?php esc_attr_e( 'Expand log details', 'siteintelix' ); ?>" aria-expanded="false" aria-controls="<?php echo esc_attr( $siteintelix_group_id ); ?>">
						...
						<div class="sitx-log-card__details" id="<?php echo esc_attr( $siteintelix_group_id ); ?>" hidden>
							<div class="sitx-log-details-grid">
								...
								<div class="sitx-stack">
									<div class="sitx-stack__head">
										...
										<div>
											<button type="button" class="sitx-debug-btn sitx-debug-btn--secondary" data-copy-text="<?php echo esc_attr( $siteintelix_stack_text ); ?>">
												<span class="dashicons dashicons-clipboard" aria-hidden="true"></span><?php esc_html_e( 'Copy', 'siteintelix' ); ?>
											</button>
											<button type="button" class="sitx-debug-btn sitx-debug-btn--secondary" disabled title="<?php esc_attr_e( 'Editor protocol is not configured.', 'siteintelix' ); ?>">
							...
							<div class="sitx-log-detail-actions">
								<button type="button" class="sitx-debug-btn sitx-debug-btn--secondary" data-copy-text="<?php echo esc_attr( $siteintelix_details_text ); ?>">
									<span class="dashicons dashicons-admin-page" aria-hidden="true"></span><?php esc_html_e( 'Copy Details', 'siteintelix' ); ?>
								</button>
								<?php if ( $siteintelix_log_has_file ) : ?>
									<a class="sitx-debug-btn sitx-debug-btn--secondary" href="<?php echo esc_url( $siteintelix_download_url ); ?>">
					...
					<?php foreach ( $siteintelix_single_entries as $siteintelix_single_entry ) : ?>
						...
						?>
						<article class="sitx-log-card sitx-single-log-card sitx-log-card--<?php echo esc_attr( $siteintelix_single_level ); ?>"
							data-log-card
							data-single-card
						...
						>
							<div class="sitx-log-card__summary">
								...
								<div class="sitx-log-card__body">
									<h3><?php echo esc_html( $siteintelix_single_title ); ?></h3>
									<div class="sitx-log-meta">
	...
	<?php if ( $siteintelix_total_pages > 1 ) : ?>
		<nav class="sitx-log-pagination" aria-label="<?php esc_attr_e( 'Debug log pagination', 'siteintelix' ); ?>">
			<span>
				<?php
				printf(
