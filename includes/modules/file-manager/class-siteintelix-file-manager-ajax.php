...
class SITEINTELIX_File_Manager_Ajax {
	...
	public static function init() {
		...
		add_action( 'wp_ajax_siteintelix_fm_extract_archive', array( __CLASS__, 'extract_archive' ) );
	...
	/** Extract a validated ZIP inside wp-content/plugins or themes. */
	public static function extract_archive() {
		self::authorize( 'siteintelix_fm_extract_archive' );
		$path   = self::post_path();
		$result = ( new SITEINTELIX_File_Manager_Extractor() )->extract( $path );
		self::audit_result( 'extract', $path, $result );
		self::respond( $result );
	}
