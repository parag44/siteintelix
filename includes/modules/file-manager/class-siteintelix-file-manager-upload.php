...
class SITEINTELIX_File_Manager_Upload {
	...
	public function store( $file, $destination, $overwrite = false ) {
		...
		if ( false === strpos( $name, '.' ) || '.' === substr( $name, -1 ) || preg_match( '/[。．｡]/u', $name ) ) {
			return $this->error( 'invalid_upload_name', __( 'Uploads must use a filename with a verified extension.', 'siteintelix' ) );
		}
		...
		if ( 'zip' === $extension ) {
			if ( ! class_exists( 'ZipArchive' ) ) {
				return $this->error( 'archive_unavailable', __( 'ZIP support is unavailable on this server.', 'siteintelix' ) );
			}
			$zip = new ZipArchive();
			if ( true !== $zip->open( $temp ) ) {
				return $this->error( 'invalid_archive', __( 'The uploaded ZIP archive is invalid.', 'siteintelix' ) );
			}
			$zip->close();
		}
