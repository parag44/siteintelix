...
class SITEINTELIX_Snippets_Admin {
	...
	private static function auth( $nonce ) {
		if ( ! SITEINTELIX_Security::can_manage_code() ) {
			wp_die( esc_html__( 'Access denied.', 'siteintelix' ), 403 );
		}
	...
	public static function render_editor() {
		...
		$id   = isset( $_GET['snippet'] ) ? absint( wp_unslash( $_GET['snippet'] ) ) : 0;
	...
	public static function render_run_once() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only confirmation selection.
		$id   = isset( $_GET['snippet'] ) ? absint( wp_unslash( $_GET['snippet'] ) ) : 0;
		$item = SITEINTELIX_Snippets_Repository::get( $id );
		include __DIR__ . '/views/run-once-confirm.php';
	}
	...
	public static function save() {
		...
		$input = SITEINTELIX_Snippets_Repository::normalize_request( $_POST );
		$valid = SITEINTELIX_Snippets_Validator::validate( $input['code'] );
		if ( is_wp_error( $valid ) ) {
			wp_safe_redirect( add_query_arg( 'snippet_error', rawurlencode( $valid->get_error_message() ), wp_get_referer() ) );
			exit;
		}
		$submit_mode = isset( $_POST['submit_mode'] ) ? sanitize_key( wp_unslash( $_POST['submit_mode'] ) ) : '';
		if ( 'save_activate' === $submit_mode ) {
			$input['status'] = 'active';
		}
		// phpcs:enable WordPress.Security.NonceVerification.Missing
		$id = $id ? ( SITEINTELIX_Snippets_Repository::update( $id, $input ) ? $id : 0 ) : SITEINTELIX_Snippets_Repository::insert( $input );
		wp_safe_redirect( add_query_arg( array( 'page' => 'siteintelix-code-snippets-new', 'snippet' => $id, 'saved' => 1 ), admin_url( 'admin.php' ) ) );
		exit;
	}
	...
	public static function action() {
		...
		// phpcs:enable WordPress.Security.NonceVerification.Recommended
		$item = SITEINTELIX_Snippets_Repository::get( $id );
		if ( $item && in_array( $do, array( 'activate', 'deactivate' ), true ) ) {
			SITEINTELIX_Snippets_Repository::set_status( array( $id ), 'activate' === $do ? 'active' : 'inactive' );
		} elseif ( $item && 'duplicate' === $do ) {
			SITEINTELIX_Snippets_Repository::duplicate( $id );
		} elseif ( $item && 'delete' === $do ) {
			SITEINTELIX_Snippets_Repository::delete( $id );
		}
		wp_safe_redirect( admin_url( 'admin.php?page=siteintelix-code-snippets' ) );
		exit;
	}
	...
	public static function bulk() {
		...
		// phpcs:disable WordPress.Security.NonceVerification.Missing -- auth() verified the action-specific nonce immediately above.
		$ids = isset( $_POST['snippet_ids'] ) ? array_map( 'absint', (array) wp_unslash( $_POST['snippet_ids'] ) ) : array();
		$do  = isset( $_POST['bulk_action'] ) ? sanitize_key( wp_unslash( $_POST['bulk_action'] ) ) : '';
		// phpcs:enable WordPress.Security.NonceVerification.Missing
		if ( in_array( $do, array( 'activate', 'deactivate' ), true ) ) {
			SITEINTELIX_Snippets_Repository::set_status( $ids, 'activate' === $do ? 'active' : 'inactive' );
		} elseif ( 'delete' === $do ) {
			foreach ( $ids as $id ) {
				SITEINTELIX_Snippets_Repository::delete( $id );
			}
	...
	public static function run_once() {
		...
		$item = SITEINTELIX_Snippets_Repository::get( $id );
		if ( $item ) {
			SITEINTELIX_Snippets_Repository::set_status( array( $id ), 'running_once' );
			SITEINTELIX_Snippets_Runner::execute( $item );
			SITEINTELIX_Snippets_Repository::set_status( array( $id ), 'inactive' );
		}
		wp_safe_redirect( admin_url( 'admin.php?page=siteintelix-code-snippets' ) );
		exit;
	}
