...
class SITEINTELIX_Snippets_Repository {
	...
	public static function set_status( $ids, $status ) {
		...
		foreach ( $ids as $id ) {
			...
			if ( 'inactive' === $status ) {
				$data['deactivated_at'] = current_time( 'mysql', true );
			}
			if ( 'active' === $status ) {
	...
	public static function active_for_scope( $scope ) {
		global $wpdb;
		$query = 'SELECT * FROM ' . self::table_name() . ' WHERE status = %s AND (scope = %s OR scope = %s) ORDER BY priority ASC, id ASC';
		return $wpdb->get_results( $wpdb->prepare( $query, 'active', $scope, 'everywhere' ), ARRAY_A );
	}
	...
	public static function record_error( $id, $error ) {
		...
		$query = 'UPDATE ' . self::table_name() . ' SET status=%s, deactivated_at=%s, error_message=%s, error_file=%s, error_line=%d, error_count=error_count+1, updated_at=%s WHERE id=%d';
		$wpdb->query( $wpdb->prepare( $query, 'inactive', current_time( 'mysql', true ), $message, $file, absint( $error->getLine() ), current_time( 'mysql', true ), absint( $id ) ) );
	}
	...
	public static function list_items( $args = array() ) {
		...
		if ( ! empty( $args['scope'] ) ) {
			$where[]  = 'scope=%s';
			$values[] = sanitize_key( $args['scope'] );
		}
		if ( ! empty( $args['recent'] ) ) {
			$where[]  = 'deactivated_at >= %s';
			$values[] = gmdate( 'Y-m-d H:i:s', time() - 7 * DAY_IN_SECONDS );
		}
