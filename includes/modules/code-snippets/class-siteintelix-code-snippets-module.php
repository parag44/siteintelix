...
class SITEINTELIX_Code_Snippets_Module {
	...
	public static function admin_notices() {
		...
		if ( SITEINTELIX_Snippets_Context::is_safe_mode() ) {
			echo '<div class="notice notice-warning"><p>' . esc_html__( 'SiteIntelix Code Snippets Safe Mode is active; no PHP snippets ran on this request.', 'siteintelix' ) . '</p></div>';
		}
		$id = absint( get_transient( 'siteintelix_snippet_recovery_notice' ) );
		if ( $id ) {
			delete_transient( 'siteintelix_snippet_recovery_notice' );
			printf( '<div class="notice notice-error"><p>%s</p></div>', esc_html( sprintf( __( 'SiteIntelix deactivated snippet #%d after a runtime error.', 'siteintelix' ), $id ) ) );
		}
	}
