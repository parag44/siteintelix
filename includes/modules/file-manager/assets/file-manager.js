(function (global) {
	...
	var contextActionMap = {
		...
		extract: { id: 'extract', label: 'Extract ZIP here', icon: 'dashicons-open-folder' },
	...
	function contextActions(item) {
		...
		return ['open', 'view', 'details', 'edit', 'download', 'extract', 'archive', 'rename', 'trash'].filter(function (id) {
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		var state = {
			path: typeof data.startPath === 'string' ? data.startPath : '',
			page: 1,
			perPage: 50,
			sort: 'name',
			order: 'asc',
		...
		function showProgress(label, percent, message, stateName) {
			var panel = select('[data-fm-progress]');
			if (!panel || state.progressDismissed) { return; }
			var value = Math.max(0, Math.min(100, Number(percent) || 0));
			panel.hidden = false;
			panel.className = 'sitx-fm-progress' + (stateName ? ' is-' + stateName : '');
			panel.dataset.progressState = stateName || 'working';
			select('[data-fm-progress-label]', panel).textContent = label || '';
			select('[data-fm-progress-percent]', panel).textContent = value + '%';
			select('[data-fm-progress-message]', panel).textContent = message || '';
			select('[data-fm-progress-bar]', panel).style.width = value + '%';
			select('[data-fm-progress-track]', panel).setAttribute('aria-valuenow', String(value));
		}

		function startProgress(label, percent, message) {
			state.progressDismissed = false;
			showProgress(label, percent, message, 'working');
		}
		...
		function handleItemAction(action, item, trigger) {
			if (action === 'open') {
				...
				openModal({
					...
					description: 'Files will be extracted into this folder. Existing files are never overwritten.',
					trigger: trigger,
					confirmLabel: 'Extract ZIP',
					onConfirm: function () {
						startProgress(item.name, 10, 'Validating archive…');
						return request('extract_archive', { path: item.path }).then(function (result) {
							var message = ((data.i18n && data.i18n.extracted) || 'ZIP extraction complete.') + ' ' + result.files + ' files extracted.';
							showProgress(item.name, 100, message, 'success');
							speak(message);
							loadDirectory();
						}).catch(function (error) {
							showProgress(item.name, 100, errorMessage(error), 'error');
							throw error;
						});
					},
				});
		...
		function setEditorStatus(status, message) {
			var element = select('[data-fm-editor-status]');
			if (!element) {
				return;
			}
			element.className = 'sitx-fm-editor__status' + (status ? ' is-' + status : '');
		...
		function refreshEditorLayout() {
			if (state.editorInstance && typeof state.editorInstance.refresh === 'function') {
		...
		function setEditorFullscreen(force) {
			var panel = select('[data-fm-editor]');
			var button = select('[data-fm-editor-fullscreen]');
			var label = select('[data-fm-editor-fullscreen-label]', button);
			var active = typeof force === 'boolean' ? force : !panel.classList.contains('is-fullscreen');
			panel.classList.toggle('is-fullscreen', active);
			document.body.classList.toggle('sitx-fm-editor-fullscreen-open', active);
			button.setAttribute('aria-pressed', active ? 'true' : 'false');
			if (label) {
				label.textContent = active
					? ((data.i18n && data.i18n.exitFullScreen) || 'Exit full screen')
					: ((data.i18n && data.i18n.fullScreen) || 'Full screen');
			}
			refreshEditorLayout();
		}
		...
		function openEditor(path) {
			request('get_file', { path: path, edit: 1 }).then(function (file) {
				...
				if (!state.editorInstance && global.wp && global.wp.codeEditor && data.editorSettings) {
					...
					state.editorInstance.on('change', markEditorDirty);
				} else if (state.editorInstance) {
					state.editorInstance.setValue(file.content);
				}
				textarea.addEventListener('input', markEditorDirty, { once: true });
				state.editorDirty = false;
				refreshEditorLayout();
		...
		function saveEditor() {
			if (!state.editorFile || state.editorSaving) {
				return;
			}
			var saveButton = select('[data-fm-editor-save]');
			state.editorSaving = true;
			saveButton.disabled = true;
			saveButton.setAttribute('aria-busy', 'true');
			setEditorStatus('saving', (data.i18n && data.i18n.saving) || 'Saving changes…');
			request('save_file', {
				...
				var message = (data.i18n && data.i18n.saved) || 'File saved successfully.';
				setEditorStatus('success', message);
				speak(message);
				loadDirectory();
			}).catch(function (error) {
				state.editorSaving = false;
				saveButton.disabled = false;
				saveButton.removeAttribute('aria-busy');
				var message = errorMessage(error) || ((data.i18n && data.i18n.saveFailed) || 'The file could not be saved.');
		...
		function uploadFiles(files, overwrite) {
			...
			list.forEach(function (file) {
				chain = chain.then(function () {
					startProgress(file.name, 0, 'Starting upload…');
					return uploadOne(file, state.path, overwrite, function (progress) {
						showProgress(file.name, progress, 'Uploading…', 'working');
						speak('Uploading ' + file.name + ': ' + progress + '%');
					}).catch(function (error) {
						failures.push(file.name + ': ' + errorMessage(error));
						showProgress(file.name, 100, errorMessage(error), 'error');
					});
				});
			});
			chain.then(function () {
				var message = failures.length ? failures.join(' ') : ((data.i18n && data.i18n.uploaded) || 'Upload complete.');
				if (!failures.length) { showProgress(list.length === 1 ? list[0].name : list.length + ' files', 100, message, 'success'); }
				speak(message);
				loadDirectory();
			});
		}
		...
		function bindBrowser() {
			...
			var progressPanel = select('[data-fm-progress]');
			var progressClose = select('[data-fm-progress-close]');
			if (progressClose) {
				progressClose.addEventListener('click', function (event) {
					event.stopPropagation();
					closeProgress();
				});
			}
			...
			document.addEventListener('pointerdown', function (event) {
				...
				if (progressPanel && !progressPanel.hidden && !progressPanel.contains(target)) {
					closeProgress();
				}
			});
			document.addEventListener('keydown', function (event) {
				if (event.key === 'Escape' && progressPanel && !progressPanel.hidden) {
					closeProgress();
				}
			});
			...
			document.addEventListener('keydown', function (event) {
				if (event.key === 'Escape' && select('[data-fm-editor]').classList.contains('is-fullscreen')) {
					setEditorFullscreen(false);
					return;
				}
