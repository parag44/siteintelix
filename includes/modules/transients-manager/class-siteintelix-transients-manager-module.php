...
class SITEINTELIX_Transients_Manager_Module {
	...
	public static function render_page() {
		...
		<div class="wrap siteintelix-wrap si-admin-wrap sitx-transients-page" id="siteintelix-transients-manager-page">
			...
			<div class="siteintelix-container sitx-transients-container">
				...
				<?php if ( $view ) : ?>
					<?php self::render_view_card( $view ); ?>
				<?php endif; ?>
	...
	private static function render_table_row( $row ) {
		...
		$view_url     = self::page_url( array( 'view_type' => $row['type'], 'view_transient' => $row['name'] ) );
		...
		<tr>
			...
			<td class="sitx-transient-actions">
				<a class="si-button si-button--ghost si-button--compact" href="<?php echo esc_url( $view_url ); ?>"><?php esc_html_e( 'View', 'siteintelix' ); ?></a>
	 ...
	 * @return void
	 */
	private static function render_view_card( $item ) {
		$value        = $item['value'];
		$serialized   = maybe_serialize( $value );
		$preview      = self::format_value_preview( $value );
		$raw_preview  = substr( $serialized, 0, self::MAX_PREVIEW_BYTES );
		$is_truncated = strlen( $serialized ) > self::MAX_PREVIEW_BYTES;
		$key          = self::row_key( $item['type'], $item['name'] );
		$txt_url      = wp_nonce_url( add_query_arg( array( 'action' => 'siteintelix_tm_export', 'type' => $item['type'], 'name' => $item['name'], 'format' => 'txt' ), admin_url( 'admin-post.php' ) ), 'siteintelix_tm_export_' . $key );
		$json_url     = wp_nonce_url( add_query_arg( array( 'action' => 'siteintelix_tm_export', 'type' => $item['type'], 'name' => $item['name'], 'format' => 'json' ), admin_url( 'admin-post.php' ) ), 'siteintelix_tm_export_' . $key );
		?>
		<section class="si-card sitx-transient-view">
			<header class="sitx-card__header">
				<div>
					<h2 class="sitx-card__title"><?php esc_html_e( 'Transient Details', 'siteintelix' ); ?></h2>
					<p class="sitx-section-desc"><?php echo esc_html( $item['name'] ); ?></p>
				</div>
				<div class="sitx-transient-actions">
					<button type="button" class="si-button si-button--secondary" data-siteintelix-copy-text="<?php echo esc_attr( $serialized ); ?>"><?php esc_html_e( 'Copy value', 'siteintelix' ); ?></button>
					<a class="si-button si-button--ghost" href="<?php echo esc_url( $txt_url ); ?>"><?php esc_html_e( 'TXT', 'siteintelix' ); ?></a>
					<a class="si-button si-button--ghost" href="<?php echo esc_url( $json_url ); ?>"><?php esc_html_e( 'JSON', 'siteintelix' ); ?></a>
				</div>
			</header>
			<div class="sitx-transient-meta-grid">
				<div><span><?php esc_html_e( 'Type', 'siteintelix' ); ?></span><strong><?php echo esc_html( 'site_transient' === $item['type'] ? __( 'Site transient', 'siteintelix' ) : __( 'Transient', 'siteintelix' ) ); ?></strong></div>
				<div><span><?php esc_html_e( 'Expiration timestamp', 'siteintelix' ); ?></span><strong><?php echo esc_html( $item['timeout'] ? (string) $item['timeout'] : __( 'None', 'siteintelix' ) ); ?></strong></div>
				<div><span><?php esc_html_e( 'Human readable expiration', 'siteintelix' ); ?></span><strong><?php echo esc_html( self::format_expiration( $item['timeout'] ) ); ?></strong></div>
				<div><span><?php esc_html_e( 'Size estimate', 'siteintelix' ); ?></span><strong><?php echo esc_html( self::format_bytes( strlen( $serialized ) ) ); ?></strong></div>
			</div>
			<details open>
				<summary><?php esc_html_e( 'Formatted preview', 'siteintelix' ); ?></summary>
				<pre class="sitx-code-preview"><?php echo esc_html( $preview ); ?></pre>
			</details>
			<details>
				<summary><?php esc_html_e( 'Raw value', 'siteintelix' ); ?></summary>
				<pre class="sitx-code-preview"><?php echo esc_html( $raw_preview ); ?></pre>
			</details>
			<?php if ( $is_truncated ) : ?>
				<p class="description"><?php esc_html_e( 'Preview truncated for safety. Use export to download the full value.', 'siteintelix' ); ?></p>
			<?php endif; ?>
		</section>
		<?php
	}
