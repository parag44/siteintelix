...
class SITEINTELIX_File_Manager_Filesystem {
	...
	public function list_directory( $path, $args = array() ) {
		...
		try {
			...
			foreach ( $iterator as $item ) {
				...
				// Enumeration may reveal a protected filename, but content and actions
				// still require their own stricter authorization checks.
				$authorized_item = $this->security->authorize_path( $absolute, $item->isDir() ? 'list' : 'list_item' );
				if ( is_wp_error( $authorized_item ) ) {
					continue;
				}
	...
	public function create_file( $parent, $name, $content = '' ) {
		...
		$settings  = SITEINTELIX_File_Manager_Settings::get();
		if ( ! in_array( $extension, (array) $settings['editable_extensions'], true ) ) {
	...
	private function actions( $absolute, $directory ) {
		...
		if ( ! $directory && 'zip' === strtolower( pathinfo( $absolute, PATHINFO_EXTENSION ) ) && ! is_wp_error( $this->security->authorize_path( $absolute, 'extract' ) ) ) {
			$actions[] = 'extract';
		}
		return $actions;
	}
