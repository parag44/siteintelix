...
class SITEINTELIX_WP_Config {
	...
	public static function is_debug_enabled_in_file() {
		...
		return (bool) preg_match( '/^[ \t]*define\s*\(\s*[\'"]WP_DEBUG[\'"]\s*,\s*true\s*\)\s*;/im', $contents );
	}

	/**
	 * Alias kept for backward-compatibility with SITEINTELIX_Debug_Source.
	 *
	 * This now reflects a plugin-managed state option that is set during
	 * successful enable/disable operations from SiteIntelix settings.
	 *
	 * @return bool
	 */
	public static function has_siteintelix_block() {
		$managed_option = defined( 'SITEINTELIX_WP_CONFIG_MANAGED_OPTION' )
			? SITEINTELIX_WP_CONFIG_MANAGED_OPTION
			: 'siteintelix_wp_config_managed';

		return (bool) get_option( $managed_option, 0 );
	}
	...
	private static function apply( $enable ) {
		...
		foreach ( self::$toggle_map as $constant => $enabled_value ) {
			...
			// Captures everything around the value so we can replace just the value.
				$pattern = '/^([ \t]*define\s*\(\s*[\'"]' . preg_quote( $constant, '/' ) . '[\'"]\s*,\s*)(' . $opposite_str . '|' . $target_str . '|1|0)(\s*\)\s*;)/im';
